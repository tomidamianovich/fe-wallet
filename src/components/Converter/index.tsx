...
const Converter: React.FC<Props> = ({ currencies, rates, t }) => {
  ...
  const handlerFromAmount = (e: React.ChangeEvent<HTMLInputElement>) =>
  ...
  const handlerConversion = () => {
    ...
    const [{ sell_rate }] = rates.filter(
      rate =>
        rate.ticker?.includes(currencyTickerFrom) &&
        rate.ticker?.includes('ARS'),
    )
    const [{ buy_rate }] = rates.filter(
      rate =>
        rate.ticker?.includes(currencyTickerTo) && rate.ticker?.includes('ARS'),
    )

...
type BalanceWithCurrencyInfo = BalanceType &
...
const BalanceDetail: React.FC<Props> = ({ balances, currencies, rates, t }) => {
  const currentBalance: BalanceType = useSelector(
    (state: CombinedState) => state.CurrentBalanceReducer,
  )
  const [cryptosBalance, setCryptosBalance] =
    useState<BalanceWithCurrencyInfo[]>()
  const [transactionWipCurrency, setTransactionWipCurrency] =
    useState<BalanceWithCurrencyInfo>(DEFAULT_BALANCE)
  const [transactionType, setTransactionType] = useState<TransactionActionType>(
    TRANSACTIONS.TYPES.BUY,
  )
  ...
  useEffect(() => {
    ...
    const cryptosBalance = balances?.reduce(
      (acc: BalanceWithCurrencyInfo[], val: BalanceType) => {
        if (!(cryptoTickers.includes(val.ticker) && !!val.amount)) return acc
        const currencyData: CurrencyType = currencies?.filter(
          currency => currency.ticker === val.ticker,
        )[0]
        const { sell_rate } = rates.find(
          rate => rate.ticker === `${val.ticker}_${currentBalance?.ticker}`,
        ) ?? { sell_rate: '0' }
        acc.push({
          ...val,
          ...currencyData,
  ...
  const handlePurchase = () => {
    ...
    let total =
      transactionType === TRANSACTIONS.TYPES.BUY
  ...
  const handleHeaderLabel = (
    balance: BalanceWithCurrencyInfo,
  ...
  const handlerDropdown = (e: EventSelect) => {
  ...
  const isPurchaseActionDisabled = () => {
    ...
    const isAccountLimitExceeded =
      amountParsed > transactionWipCurrency?.account_limit
    switch (transactionType) {
      case TRANSACTIONS.TYPES.BUY:
        return (
          amountIsZero ||
          isAccountLimitExceeded ||
          amountParsed * parseFloat(transactionWipCurrency?.sell_rate) >
